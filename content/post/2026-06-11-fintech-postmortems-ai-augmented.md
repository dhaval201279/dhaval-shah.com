...
**Decision made:** Route all payment status checks through synchronous REST calls to a downstream fraud scoring service, co-located in the same data center  
...
# POST-MORTEM 2: The Storage Model That Optimized Its Way Into a Corner
## Incident Summary
...
**Phase 2 - The optimization: virtual columns on JSON fields**

The team's response was technically reasonable: extracting the most-queried JSON fields as Oracle virtual columns, index them, and route lookup queries through the indexed virtual columns rather than full JSON path expressions.
...
3. **Operational brittleness during schema evolution.** When a new payment method (EMI via credit card) introduced a new nested field - `$.paymentDetail.emiInfo.tenure` - that needed a virtual column, the `ALTER TABLE` added the virtual column on a live table with 800K+ rows caused a prolonged lock wait during a period of active write traffic.
...
**Reasoning at the time:** "Microservices at our current scale is operational overhead we can't afford. We'll modularize internally and split later."  

**What happened:**
The internal package structure was held for a year. Then two things happened simultaneously: the engineering team grew from < 10 to > 20 engineers, and a regulatory requirement mandated that PCI-DSS scope be isolated from non-payment transaction processing.
...
**Remediation cost:** A 6-month extraction program to isolate PCI scope. Database schema decomposition required a 2 months of dual-write migration period with both services running simultaneously against a shared schema under active traffic.
...
The prompts are not the answer. They are question generators. Your answers - drawn from your system's specific constraints, your team's operational reality, and your own production experience - are the record that matters.
