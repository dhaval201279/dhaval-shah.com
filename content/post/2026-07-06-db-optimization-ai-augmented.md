...
tags:
  ...
  - db-optimization
  - ai-augmented-se
thumbnail: "images/wp-content/uploads/2026/07/gc-comparison-ai-review.png"
---

[![](https://www.dhaval-shah.com/images/wp-content/uploads/2026/07/gc-comparison-ai-review.png)](https://www.dhaval-shah.com/images/wp-content/uploads/2026/07/gc-comparison-ai-review.png)
-----------------------------------------------------------------------------------------------------------------------------------------

## Background

[Week 1's post-mortem](https://dhaval-shah.com) walked through an Oracle JSON storage problem: a payment transaction table stored as JSON, a reconciliation query that needed to filter on a nested field, and the chain of fixes - virtual columns, then a full move to columnar storage — that the team eventually needed.
...
To make this concrete rather than theoretical, I built a 3-million-row Postgres table with the same shape of payment data as the Oracle example — JSONB payload, nested merchant category, card network, risk signals — seeded with realistic skew rather than uniform random data, and ran real queries against it. Every plan and timing number in this post is captured directly from that table. Nothing here is illustrative or simulated.
...
3 million rows, generated with deliberate skew rather than even distribution, because even distribution hides exactly the kind of problem this post is about:
...
This matters for everything that follows. A query filtering on FASHION and a query filtering on HEALTHCARE are touching the same column with the same index, but they're asking fundamentally different questions of the planner — one wants 1.1 million rows out of 3 million, the other wants 6,174. The right strategy for one is often the wrong strategy for the other, and a single index doesn't automatically mean a single plan shape.
...
879ms. The plan is a parallel sequential scan - Postgres reads through the entire table across 2 worker processes, evaluating the JSON path expression on every single row, throwing away 998,561 rows per worker to find the 100 it actually wants. `Buffers: shared hit=15536 read=142439` - roughly 158,000 8KB blocks touched, which on a 1.2GB table is close to reading the whole thing.

This is the same fundamental problem the Oracle post-mortem described: querying inside a nested JSON structure with no supporting index means the database has no way to narrow down the search before it starts. It has to look at everything.
...
879ms down to 49.9ms. A 17.6x improvement, buffer touches down from 158,000 to about 6,000. One index, no application code changes, no schema migration.
...
Now watch what happens when the same query, same index, asks for a *common* category instead of a rare one:
...
2.9ms — even faster — but the plan isn't using `idx_txn_category` at all. It's using the `created_at` index instead, and just filtering FASHION rows out of whatever it finds along the way. The planner worked out that with FASHION at 37% of the table, the category index wouldn't actually narrow things down much, so scanning by date and filtering is cheaper than bouncing through the category index at all.

This is the planner doing exactly what it's supposed to do — picking different strategies for different selectivity. It also means "I added an index" is not the same claim as "this query is fast now." The same index can be the right tool for one filter value and irrelevant for another, on the exact same table, same query structure. Anyone tuning this in production needs to test across the actual range of values the application will query for, not just the one that happened to be slow in the bug report.
...
First: `Sort Method: external merge Disk: 2776kB`. The sort needed for the aggregation didn't fit in memory and spilled to disk. That's controlled by `work_mem`, and it's worth checking before assuming the query itself is the problem - the same query with more memory available to the sort might not spill at all.

Second: `Heap Blocks: exact=45866 lossy=33446`. "Lossy" means Postgres ran out of room in its bitmap to track individual rows precisely and had to fall back to tracking whole pages instead — meaning it then had to recheck every row on those pages individually, which is where `Rows Removed by Index Recheck: 613652` comes from. That's controlled by `work_mem` too. Status alone matches 2.1 million rows (SUCCESS is 90% of the table) — combining it with the date filter via `BitmapAnd` still leaves a large enough candidate set that the bitmap representation runs out of precision.

Neither of these is something a single CREATE INDEX statement fixes. This is the kind of query where the actual lever is a memory setting, not an index - and that's a different diagnosis than the HEALTHCARE lookup needed.
...
The prompt I used asks for five specific things: walk the actual cost and timing numbers in the plan rather than describing the query in general terms; identify whether the bottleneck is a missing index, a planner selectivity decision, or a memory-driven spill; explain anything unusual in the plan output specifically (Recheck Cond, lossy blocks, external merge) rather than skipping past it; state what would happen if the same query ran against a different value with different selectivity; and propose a specific, testable next step rather than a generic "add an index" response.

The first pass at this, run against the HEALTHCARE plan alone, gave a reasonable explanation of the speed-up but didn't flag the `Recheck Cond` line at all - it read past it as part of normal index usage rather than noting what it actually meant. Once asked specifically what "Recheck Cond" means and why it's there, the explanation held up: a JSON path index in Postgres returns candidate rows, the database still needs to verify the JSON value against the live row data, and that recheck step is real work, not free.

The same approach against the reconciliation query did better on its own - `work_mem` and the disk spill came up without prompting, likely because "external merge Disk" is a distinctive enough phrase to stand out. But it had no way of knowing what `work_mem` was actually set to in this environment, or whether raising it was safe given everything else running on the same instance. That context has to come from whoever's running the database, not from the plan text alone.
...
The Oracle post-mortem from Week 1 and this Postgres example are not the same problem with the same fix. Oracle's virtual columns introduce a real write-side cost — every insert and update re-evaluates the virtual column expression. Postgres's expression index doesn't have that exact failure mode, but it has its own nuance: the same index can produce a completely different plan depending on how selective the filter value is, and a fix that works for a rare value can be irrelevant for a common one on the identical query.

What's consistent across both databases is the underlying lesson. A storage format that looks flexible and harmless at low row counts can hide real query cost that only shows up at scale — and the only way to know which fix actually applies is to read the real plan, not assume from the table schema what the database is doing underneath it.
